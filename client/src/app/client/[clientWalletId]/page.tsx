...
export default function TransactionsPage() {
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return filtered.map(transaction => ({
      ...transaction,
      realTimeUSD: convertToUSD(parseFloat(transaction.amount), data.adminWallet.currency),
      currentPrice: getCoinData(data.adminWallet.currency)?.current_price || 0
    }));
  }, [data, statusFilter, typeFilter, convertToUSD, getCoinData]);
  ...
  const coinData = data ? getCoinData(data.adminWallet.currency) : null;
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Viewing all transactions for your <strong>{data?.adminWallet.currency} Wallet</strong>
              </p>
              <p className="text-gray-600 mt-1">
                Total Balance: <strong>${data?.amountInUSD}</strong>
              </p>
            </div>
            ...
            {coinData && (
              <div className="text-right">
                <div className="flex items-center gap-2">
                  {coinData.image && (
                    <img src={coinData.image} alt={data?.adminWallet.currency} className="w-8 h-8 rounded-full" />
                  )}
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      Current {data?.adminWallet.currency} Price
                    </p>
                  </div>
                </div>
              </div>
          ...
          {filteredTransactions.length === 0 ? (
            ...
            <div className="divide-y divide-gray-200">
              {filteredTransactions.map((transaction) => {
                const valueDifference = transaction.realTimeUSD - transaction.amountInUSD;
                      ...
                      <div className="text-right">
                        <div className="flex items-center space-x-3">
                          <div className="text-right">
                            {/* Crypto Amount */}
                            <p className="text-lg font-bold text-gray-900">
                              {transaction.amount} {data?.adminWallet.currencyAbbreviation}
                            </p>
                            ...
                            <p className="text-sm font-semibold text-green-600">
                              ${transaction.realTimeUSD.toFixed(2)}
                            </p>
                        ...
                        {transaction.fee && (
                          <p className="text-xs text-gray-500 mt-1">
                            Fee: {transaction.fee} {data?.adminWallet.currencyAbbreviation}
                          </p>
                        )}
                      </div>
                    </div>
          ...
          <div className="mt-8 grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="bg-white rounded-lg shadow p-4">
              ...
              <p className="text-2xl font-bold text-green-600">
                ${filteredTransactions.reduce((sum, t) => sum + t.realTimeUSD, 0).toFixed(2)}
              </p>
